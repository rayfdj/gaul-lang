...
fn fs_module() -> HashMap<String, Value> {
    ...
    m.insert("read_file".into(), native("read_file", Some(1), |args| {
        match &args[0] {
    ...
    m.insert("write_file".into(), native("write_file", Some(2), |args| {
        match (&args[0], &args[1]) {
    ...
    m.insert("append_file".into(), native("append_file", Some(2), |args| {
        match (&args[0], &args[1]) {
            (Value::Str(path), Value::Str(content)) => {
                use std::io::Write;
    ...
    m.insert("file_exists".into(), native("file_exists", Some(1), |args| {
        match &args[0] {
...
fn sys_module() -> HashMap<String, Value> {
    ...
    m.insert("args".into(), native("args", Some(0), |_args| {
    ...
    m.insert("exit".into(), native("exit", Some(1), |args| {
        match &args[0] {
    ...
    m.insert("clock".into(), native("clock", Some(0), |_args| {
    ...
    m.insert("env_get".into(), native("env_get", Some(1), |args| {
        match &args[0] {
    ...
    m.insert("env_set".into(), native("env_set", Some(2), |args| {
        match (&args[0], &args[1]) {
